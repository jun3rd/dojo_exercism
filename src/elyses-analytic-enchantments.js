...
export function getCardPosition(stack, card) {
	console.log(stack)
	console.log(card)
	let foundIndex = stack.findIndex( lookingFor => {
		return (lookingFor === card) ? true : false
	})
	return foundIndex
}
...
export function isEachCardEven(stack) {
	return stack.every(card => (card % 2 === 0) ? true : false)
}
...
export function doesStackIncludeOddCard(stack) {
	return stack.some(card => (card % 2 > 0) ? true : false)
}
...
export function getFirstOddCard(stack) {
	return stack.find((card) => (card % 2 > 0) ? true : false)
}
...
export function getFirstEvenCardPosition(stack) {
	return stack.findIndex((card) => (card % 2 === 0) ? true : false)
}
